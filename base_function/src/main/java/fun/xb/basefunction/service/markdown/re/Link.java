...
public class Link {
    private static Pattern Order_PATTERN = Pattern.compile("\\[(.*?)]\\((.*?)\\)",Pattern.MULTILINE);//使用多行匹配才能使用 $和^符号

    static String url="";

    static String linkT="</a>";
    ...
    public static String mdLinkparse(String markdown) {
        ...
        while (matcher.find()) {// 如果符合条件，处理它的子内部而不是外部
            ...
            String context=matcher.group(1);
             url=matcher.group(2);
            String link="<a class=\"md_link\" href=\""+url+"\">";
            String re=link+context+linkT;
            matcher.appendReplacement(sb, re);//这样的做法可以更为精细的处理每一个匹配
        }
